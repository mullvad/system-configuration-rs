...
impl<T> SCDynamicStoreBuilder<T> {
    ...
    pub fn build(mut self) -> SCDynamicStore {
        ...
        if let Some(callback) = self.callback_context.take() {
            // move the callback struct to the heap and "forget" it
            let info_ptr = Box::into_raw(Box::new(callback));
