...
bitflags::bitflags! {
    ...
    pub struct ReachabilityFlags: u32 {
        ...
        /// The specified node name or address can be reached using the current network configuration, but a connection
        /// must first be established. Any traffic directed to the specified name or address will initiate the
        /// connection.
        ...
        /// Network traffic to the specified node name or address will not go through a gateway, but is routed directly
        /// to one of the interfaces in the system.
...
impl SCNetworkReachability {
    ...
    /// function multiple times will clear the subsequently set callback. Returns false if
    /// setting a callback failed.
    ///
    ...
    pub fn set_callback<F: Fn(ReachabilityFlags) + Clone + Send>(
        &mut self,
        callback: F,
...
#[derive(Clone)]
struct NetworkReachabilityCallbackContext<T: Fn(ReachabilityFlags) + Clone + Send> {
    _host: SCNetworkReachability,
    callback: T,
}

impl<T: Fn(ReachabilityFlags) + Clone + Send> NetworkReachabilityCallbackContext<T> {
    ...
    extern "C" fn release_context(ctx: *const c_void) {
        unsafe {
            let _: Box<Self> = Box::from_raw(ctx as *mut _);
        }
    }

    extern "C" fn retain_context(ctx_ptr: *const c_void) -> *const c_void {
        unsafe {
            let ctx_ref: &Self = &*(ctx_ptr as *const Self);
            let new_ctx: Box<Self> = Box::new(ctx_ref.clone());
            Box::into_raw(new_ctx) as *const c_void
        }
    }
}
