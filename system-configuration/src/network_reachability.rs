...
impl SCNetworkReachability {
    ...
    pub fn from_host(host: &CStr) -> Result<Self, FromHostError> {
        ...
        if ptr.is_null() {
            ...
            unsafe { Ok(Self::wrap_under_create_rule(ptr)) }
        }
    }
...
fn to_c_sockaddr(addr: SocketAddr) -> Box<libc::sockaddr> {
    unsafe {
        match addr {
            SocketAddr::V4(addr) => std::mem::transmute(Box::new(libc::sockaddr_in {
            ...
            SocketAddr::V6(addr) => std::mem::transmute(Box::new(libc::sockaddr_in6 {
